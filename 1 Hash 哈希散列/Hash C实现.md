...
HashTable *FindHashbyKey(int key) {   
    Hashtable *dstpHash);                   /* 定义 pDstHash */
...
函数使用：
```c
HASH_ADD_INT(pExistHash, key, pDstHash);  /* key: name of key field */
```
**参数含义**：
...
void InsertHashbyKey(int key, char *name) {
    ...
    /* 判断key值是否已经存在? */
    HASH_FIND_INT(pHashInsert, &key, pKeyValuePair); 
    /* 判断key值是否不已经存在 */
    if (pKeyValuePair == NULL) {
      pKeyValuePair = (HashTable *)malloc(sizeof *HashTable);
      pKeyValuePair->key = key;
      /* key: name of key field */
      HASH_ADD_INT(pHashInsert, key, pKeyValuePair);  
    }
